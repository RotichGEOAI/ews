...
1. **Push the project to GitHub** (Streamlit Cloud deploys from a GitHub repo,
   ...
   git remote add origin https://github.com/<your-username>/north-rift-geoai-ews.git
...
3. **Click "New app"**, then:
   - Repository: `<your-username>/north-rift-geoai-ews`
   - Branch: `main`
...
sit at the **root of your GitHub repository** — not just inside this project
folder — if your repo nests this project under a subfolder (as in
`geoai_ews/streamlit_app/streamlit_app.py`). Verify with:
```bash
git ls-files | grep -E "requirements|packages.txt|runtime.txt"
```
If they only show up under a subfolder path, either move them to the true
repo root, or set the dependencies file path explicitly in Streamlit Cloud's
Advanced settings.
